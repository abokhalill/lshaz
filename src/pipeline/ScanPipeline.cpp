...
ScanResult ScanPipeline::execute(const ScanRequest &request) {
    ...
    if (request.filter.maxFiles > 0 &&
        sources.size() > request.filter.maxFiles) {
        sources.resize(request.filter.maxFiles);
    }
...
ScanResult ScanPipeline::executeLegacy(
        ...
        const clang::tooling::CompilationDatabase &compDB,
        const std::vector<std::string> &sources) {
    std::vector<std::string> filtered = sources;
    if (request.filter.maxFiles > 0 &&
        filtered.size() > request.filter.maxFiles) {
        filtered.resize(request.filter.maxFiles);
    }
